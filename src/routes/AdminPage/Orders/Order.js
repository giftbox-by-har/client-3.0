...
const Order = () => {
	...
	const renderOrderStatusOptions = (orderId, currentStatus) => {
		const statusOptions = [
			"Menunggu konfirmasi",
			"Pesanan diterima",
			"Diproses",
						...
						>
							<div style={{ width: "100%" }}>
								...
								>
									...
									<div>
										<div>
											{renderOrderStatusOptions(order._id, order.status)}
										</div>
									</div>
								</div>
								<div>
								...
								{order.customPackages.map((customPackage) => (
									<div key={customPackage._id} className="customgiftpage3-box">
										<div style={{ display: "flex", alignItems: "center" }}>
											{customPackage.customPackage.boxType.images &&
											customPackage.customPackage.boxType.images.length > 0 ? (
												<img
